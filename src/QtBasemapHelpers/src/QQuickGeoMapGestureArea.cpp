...
**
****************************************************************************/
#include "QQuickGeoMapGestureArea.h"
...
static qreal angleDelta(const qreal angle1, const qreal angle2)
{
    ...
    if (delta
        > 180.0)  // detect crossing angle1 positive, angle2 negative, rotation counterclockwise, difference negative
        delta = angle1 - angle2 - 360.0;
    else if (delta
             < -180.0)  // detect crossing angle1 negative, angle2 positive, rotation clockwise, difference positive
        delta = angle1 - angle2 + 360.0;
    return delta;
}
static bool pointDragged(const QPointF& pOld, const QPointF& pNew)
{
    ...
    return (qAbs(pNew.x() - pOld.x()) > startDragDistance || qAbs(pNew.y() - pOld.y()) > startDragDistance);
}
...
static bool movingParallelVertical(const QPointF& p1old,
                                   const QPointF& p1new,
                                   const QPointF& p2old,
                                   const QPointF& p2new)
{
...
/*!
    ...
    Map {
        ...
        gesture.onPinchFinished:{
            ...
            console.log("        Points (" + gesture.point1.x + ", " + gesture.point1.y + ") - (" + gesture.point2.x +
   ", " + gesture.point2.y + ")") console.log("   Coordinates (" + coordinate1.latitude + ", " + coordinate1.longitude +
...
/*!
    ...
    \since QtLocation 5.9
*/
QQuickGeoMapGestureArea::QQuickGeoMapGestureArea(QDeclarativeGeoMap* map, QQuickItem* parent)
      : QQuickItem(parent)
      , m_map(0)
      ...
      , m_preventStealing(false)
{
    ...
    if (m_declarativeMap)
    {
        if (!m_declarativeMap->mapReady())
        {
            connect(m_declarativeMap, &QDeclarativeGeoMap::mapReadyChanged, this, [this](bool isReady) {
                if (isReady)
                {
                    setMap(m_declarativeMap->map());
                }
            });
        }
        else
...
void QQuickGeoMapGestureArea::setMap(QGeoMap* map)
{
    ...
    connect(m_flick.m_animation,
            &QQuickAbstractAnimation::stopped,
            this,
            &QQuickGeoMapGestureArea::handleFlickAnimationStopped);
...
void QQuickGeoMapGestureArea::mousePressEvent(QMouseEvent* event)
{
    handleMousePressEvent(event);
}

void QQuickGeoMapGestureArea::mouseMoveEvent(QMouseEvent* event)
{
    handleMouseMoveEvent(event);
}

void QQuickGeoMapGestureArea::mouseReleaseEvent(QMouseEvent* event)
{
    handleMouseReleaseEvent(event);
    emit clicked(event->pos(), event->button());
}

void QQuickGeoMapGestureArea::mouseUngrabEvent()
{
    handleMouseUngrabEvent();
}

void QQuickGeoMapGestureArea::touchUngrabEvent()
{
    handleTouchUngrabEvent();
}

void QQuickGeoMapGestureArea::touchEvent(QTouchEvent* event)
{
    handleTouchEvent(event);
}

void QQuickGeoMapGestureArea::mouseDoubleClickEvent(QMouseEvent* event)
{
    emit doubleClicked(event->pos(), event->button());
}

void QQuickGeoMapGestureArea::hoverMoveEvent(QHoverEvent* event)
{
    emit positionChanged(event->position());
}

#if QT_CONFIG(wheelevent)
void QQuickGeoMapGestureArea::wheelEvent(QWheelEvent* event)
{
...
bool QQuickGeoMapGestureArea::hoverEnabled() const
{
    return acceptHoverEvents();
}

void QQuickGeoMapGestureArea::setHoverEnabled(bool value)
{
    if (value != acceptHoverEvents())
    {
...
QTouchEvent::TouchPoint* createTouchPointFromMouseEvent(QMouseEvent* event, Qt::TouchPointState state)
{
    ...
    QTouchEvent::TouchPoint* newPoint = new QTouchEvent::TouchPoint(
        0, static_cast<QTouchEvent::TouchPoint::State>(state), event->scenePosition(), event->globalPosition());
    return newPoint;
}
/*!
    \internal
...
void QQuickGeoMapGestureArea::handleTouchEvent(QTouchEvent* event)
{
    ...
    m_touchPoints.clear();
    m_mousePoint.reset();
    for (int i = 0; i < event->points().count(); ++i)
    {
        auto point = event->points().at(i);
        if (point.state() != static_cast<quint8>(Qt::TouchPointReleased))
            m_touchPoints << point;
    }
...
void QQuickGeoMapGestureArea::startOneTouchPoint()
{
    ...
    m_startCoord.setLongitude(m_startCoord.longitude() + startCoord.longitude() - m_touchCenterCoord.longitude());
    m_startCoord.setLatitude(m_startCoord.latitude() + startCoord.latitude() - m_touchCenterCoord.latitude());
}
/*!
    \internal
*/
...
void QQuickGeoMapGestureArea::startTwoTouchPoints()
{
    ...
    m_startCoord.setLongitude(m_startCoord.longitude() + startCoord.longitude() - m_touchCenterCoord.longitude());
    m_startCoord.setLatitude(m_startCoord.latitude() + startCoord.latitude() - m_touchCenterCoord.latitude());
    m_twoTouchAngleStart =
        touchAngle(m_sceneStartPoint1, m_sceneStartPoint2);  // Initial angle used for calculating rotation
...
void QQuickGeoMapGestureArea::tiltStateMachine()
{
    ...
    case tiltInactive:
        ...
        {
            if (!isRotationActive() && !isPinchActive() && canStartTilt())
            {  // only gesture that can be overridden: pan/flick
...
void QQuickGeoMapGestureArea::pinchStateMachine()
{
    ...
    case pinchActive:
        if (m_allPoints.count() <= 1)
        {  // Once started, pinch goes off only when finger(s) are release
            setPinchState(pinchInactive);
            m_declarativeMap->setKeepMouseGrab(m_preventStealing);
            m_declarativeMap->setKeepTouchGrab(m_preventStealing);
            endPinch();
...
void QQuickGeoMapGestureArea::updatePinch()
{
    ...
    if (m_acceptedGestures & PinchGesture)
    {
        ...
        qreal perPinchMinimumZoomLevel =
            qMax(m_pinch.m_zoom.m_start - m_pinch.m_zoom.maximumChange, m_pinch.m_zoom.m_minimum);
        qreal perPinchMaximumZoomLevel =
            qMin(m_pinch.m_zoom.m_start + m_pinch.m_zoom.maximumChange, m_pinch.m_zoom.m_maximum);
...
void QQuickGeoMapGestureArea::panStateMachine()
{
    ...
    case flickActive:
        if (m_allPoints.count() > 0)
        {  // re touched before movement ended
            stopFlick();
            m_declarativeMap->setKeepMouseGrab(true);
            setFlickState(panActive);
        }
...
bool QQuickGeoMapGestureArea::canStartPan()
{
    if (m_allPoints.count() == 0 || (m_acceptedGestures & PanGesture) == 0
        || (m_mousePoint
            && m_mousePoint->state()
                   == static_cast<quint8>(Qt::TouchPointReleased)))  // mouseReleaseEvent handling does not
        // clear m_mousePoint, only ungrabMouse does -- QTBUG-66534
        return false;
